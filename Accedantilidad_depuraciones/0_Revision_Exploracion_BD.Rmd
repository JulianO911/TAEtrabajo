...
Y como se observa, esta base de datos posee un total de `r filas`, cada una con diecisiete características: año, CBML, clase de accidente, dirección, dirección encasillada, diseño, expediente, fecha de accidente, fecha de accidentes, mes, número de radicado, número de coomuna, barrio, comuna, ubicación, coordenada x de la ubicación y coordenada y de la ubicación. Revisando esto, se comienzan a notar un par de inconvenientes con la base de datos: el primero es que es hay diecisiete variables a pesar de que el reporte de MeData incluye dieciocho variables en total, y lo segundo es que a partir de la variable titulada como *mes* hay un error en el nombre de las columnas. Nótese que la ausencia de una columna y el corrimiento en los nombres de algunas columnas pueden estar correlacionados. Así, por ejemplo, se tiene que en "MES" se está describiendo si en un accidente hubo heridos, daños materiales o fallecimientos, mientras que para la característica "COMUNA" en realidad se está describiendo el barrio en el que se dio el incidente, por lo que se procede a su arreglo. Con esto presente, se comienza corrigiendo los nombres de las columnas en términos de los nombres de las columnas originales.
...
Entonces, al chequear nuevamente la base de datos, como se ve a continuación, se verifica que los nombres de las características de esta base de datos ahora tienen relación con los niveles que incluye cada una.

```{r Estructura dados nuevos nombres, echo = FALSE}
glimpse(accidentes)
```

Con esto listo, se procede a la verificación de cada una de las variables de esta base de datos para proceder con su respectiva corrección, incluyendo también una modificación en el tipo de variable que registra $\color{blue}{\textsf{R}}$ para cada variable, ya que como se observa, la mayoría de ellas son almacenadas como tipo carácter ($\texttt{chr}$), cuando debería ser realmente de tipo factor, fecha o numérico.
...
Y se constanta que existen errores en la codificación de este archivo, por lo que se procede a su corrección, así como a cambio de errores ortográficos en algunos niveles, con lo que se obtienen los siguientes nuevos niveles de medición:
...
Entonces, al filtrar la base de datos para estudiar las duplicaciones en los números de radicos, se evidencia que no existen dos observaciones exactamente iguales, sino que difieran en la fecha y el barrio, por dar un par de ejemplos, de manera que no es necesario descartar observaciones por cuenta del número de radicado.
...
```{r}
barriosAux <- accidentes %>% # Vector con todos los barrios de la BD
  select(c(BARRIO,COMUNA))
barrios <- barriosAux %>% 
  group_by(BARRIO,COMUNA) %>% 
  summarise(Frecuencia = n())
barrios_comuna <- barrios[!is.na(barrios$BARRIO) & !is.na(barrios$COMUNA),]
barrios_comuna <- barrios_comuna %>% # Vector con todos los barrios de la BD
  select(c(BARRIO,COMUNA))

barrios_comuna
```
```{r}
train <- accidentes[accidentes$BARRIO %in% barrios_comuna$BARRIO,]
test <- accidentes[!(accidentes$BARRIO %in% barrios_comuna$BARRIO),]
```

```{r}
get_Barrio <- function(X_0, Y_0, n){
  k <- round(sqrt(n))
...
```
```{r}
test$BARRIO <- get_Barrio(test$X, test$Y,20233) 
```

```{r}
accidentes[!(accidentes$BARRIO %in% barrios_comuna$BARRIO),]$BARRIO <- test$BARRIO
```
