...
defModeloyPlot <- function(clase,caracteristicas,K){
  ...
  resultado_plot <- pivot_longer(resultado, 
   ...
   plot <- ggplot(data = resultado_plot,
         ...
         color = curva)) +
    geom_line() +
    ggtitle(paste("Datos observados vs datos predichos para",tolower(clase),sep = " ")) +
...
modelos("Choque")
```
Para predecir los accidentes de choque se utilizarán 4 características y un K=11.
...
modelos("Atropello")
```
Este modelo utilizará 4 características y un k=6.
...
modelos("Volcamiento")
```
Con la predicción de volcamientos se utilizarán 6 características y un k=12.
...
```
Respecto a caída del ocupante se usarán 6 características y un k=12.
...
modelos("Otro")
```
Por último, en el caso de otros accidentes, se utilizaran 3 características un k=13.
...
comparacion2020 <- function(modelo,clase,i){
  ...
  plot <- ggplot(data = resultado_plot,
         ...
         color = curva)) +
    geom_line() +
    ggtitle(paste("Datos observados vs datos predichos en 2020 para",tolower(clase),sep = " ")) +
    labs(y = "Total de accidentes", x = "Fecha") + 
    theme(plot.title = element_text(hjust = 0.5)) +
    labs(color = "Curva") + 
    scale_color_manual(values=c("Red", "Blue"))
...
```{r include=FALSE}
save(modeloChoque,file = "ModeloChoque.RData")
save(modeloAtropello,file = "ModeloAtropello.RData")
save(modeloVolcamiento,file = "ModeloVolcamiento.RData")
save(modeloCaida,file = "ModeloCaida.RData")
save(modeloIncendio,file = "ModeloIncendio.RData")
save(modeloOtro,file = "ModeloOtro.RData")
```
